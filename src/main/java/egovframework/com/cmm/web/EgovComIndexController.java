...
public class EgovComIndexController implements ApplicationContextAware, InitializingBean {
	...
	@Resource(name="PhcfMenuService")
	private PhcfMenuService service;
...
//			/*@Controller Annotation 처리된 클래스를 모두 찾는다.*/
	...
	@SuppressWarnings("unchecked")
	@RequestMapping("/PhcfLeft.do")
	public String setPhcfLeftMenu(ModelMap model) throws Exception {
		System.out.println("=== 1.여기는왔나?");
		Map<String, Object> map = (Map<String, Object>) service.selectMenuList();
		System.out.println("2. 여기는왔나? 맵은? === : " + map);
		model.addAttribute("resultList", map);
		
		LOGGER.debug("EgovComIndexController index is called ");

		return "egovframework/phcf/com/PhcfUnitLeft";
	}
}
