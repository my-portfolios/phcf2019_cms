...
<div class="wTableFrm">
	...
	<tbody>
		...
		<!-- 게시판 템플릿 -->
		<c:set var="title"><spring:message code="comCopBbs.boardMasterVO.regist.tmplatNm"/> </c:set>
		<tr>
		...
		<c:set var="title"><spring:message code="comCopBbs.boardMasterVO.regist.acYn"/> </c:set>
		<tr>
			<th><label for="acYn">${title } <span class="pilsu">*</span></label></th>
			<td class="left">
				<form:select path="acYn" title="${title} ${inputTxt }" cssClass="txt">
					<form:option value="" label="--선택하세요--" />
					<form:option value="Y"  label="예" />
	  		   		<form:option value='N'>아니오</form:option>
				</form:select>
				<div><form:errors path="acYn" cssClass="error" /></div>       
			</td>
		</tr>
		
		<!-- 추가칼럼정의 -->
		<c:forEach var="epoch" begin="1" end="20" step="1" >
			<c:set var="title"><spring:message code="comCopBbs.boardMasterVO.regist.addedColmns"/>${epoch} </c:set>
			<c:set var="var">ac${epoch}Nm </c:set>
			<tr class="addedColmnsDiv" style="display:none;">
				<th><label for="${var}">${title} 정의</label></th>
				<td class="left">
				    <form:input path="${var}" title="${title} ${inputTxt}" class="addedColmns" size="70" maxlength="70" />
	   				<div><form:errors path="${var}" cssClass="error" /></div>     
				</td>
			</tr>
		</c:forEach>
...
<script>
/* var skinList = <c:out value='${TmpltKdList}' escapeXml='false' />;
$("#tmplatId").change(function(){
	$("#skinId").empty();
	var templateId= $("#tmplatId").val();
	if(templateId){
		skinList[templateId].forEach(function(item, index, array){
			var option = $("<option value='"+item+"'>"+item+"</option>");
			$("#skinId").append(option);
		});
	}
}); */
$("#acYn").change(function(){
	if($("#acYn").val()=='Y'){
		$(".addedColmnsDiv").show();
		$(".addedColmns").attr("disabled",false);
	}
	else{
		$(".addedColmnsDiv").hide();
