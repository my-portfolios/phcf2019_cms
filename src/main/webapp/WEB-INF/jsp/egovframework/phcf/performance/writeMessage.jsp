<%
/**
 * @Class Name : EgovMailRegist.jsp / EgovSmsInfoRegist.jsp
 	-> writeMessage.jsp (by 김경민)
 ...
 *  @since 2009.03.11
 *  @version 1.0
 *  @see EgovMailRegist.jsp / EgovSmsInfoRegist.jsp
 *
 */
%>
...
<c:set var="pageTitle">안내 문자 메세지</c:set>
<!DOCTYPE html>
<html>
<head>
 ...
 * 등록 처리 함수
 ******************************************************** */
function fn_egov_write_message(form) {

	if(!validateSndngMailVO(form)){
		return false;
	}else{
		if(confirm("<spring:message code="common.regist.msg" />")){	
			form.action = "<c:url value='/performance/sendTextMessage.do' />";
			form.submit();
		}
	}
}
...
<div class="wTableFrm">
	<!-- 타이틀 -->
	<h2>${pageTitle} 작성</h2>
	<form:form name="sndngMailVO" method="post" enctype="multipart/form-data" action="${pageContext.request.contextPath}/performance/sendTextMessage.do" onSubmit="fn_egov_write_message(document.forms[0]); return false;"> 
...
<%-- 			     <div><form:errors path="recptnPerson" cssClass="error" /></div> --%>
		...
		<tr>
			...
			<td class="left">
			    <input name="trnsmitTelno" type="text"/>
				<br />
			</td>
		</tr>
		...
		<!-- 전송내용 -->
		<c:set var="title"><spring:message code="cop.sms.trnsmitCn" /></c:set>
		<tr>
			<th>${title} 
			</th>
			<td class="nopd">
				<textarea id="emailCn" name="emailCn" style="width: 80%; height: 150px;" tabindex="4" title="${title} ${inputTxt}" ></textarea>
			</td>
		</tr>
	</tbody>
	</table>
